...
# 延迟导入 DatabaseService，避免 sqlcipher DLL 问题
if TYPE_CHECKING:
...
def create_app(db: "DatabaseService", user_info: UserInfo, config: dict) -> FastMCP:
    ...
    @mcp.tool()
    @handle_tool_exceptions
    def query_contacts(ctx: Context, query: Optional[str] = None) -> str:
        """
        查询微信联系人列表。
        支持按昵称或备注搜索，不传query返回所有联系人。
        """
        app_context = _get_app_context_from_request(ctx)
    ...
    @mcp.tool()
    @handle_tool_exceptions
    def get_contact_info(ctx: Context, identifier: str) -> str:
        """
        获取指定联系人的详细信息。
        支持用户名、昵称、备注等多种方式查找。
        """
        app_context = _get_app_context_from_request(ctx)
        contact, error = _find_contact_by_identifier(app_context.db, identifier, "any")
        if error:
    ...
    @mcp.tool()
    @handle_tool_exceptions
    def get_decryptor_status(ctx: Context) -> str:
        """
        获取解密器状态信息。
        返回密钥数量、dbkey 状态等。
        """
        ...
        return json.dumps({
            "status": "initialized",
            "keys_count": len(decryptor.keys),
            "dbkey_set": bool(decryptor._global_dbkey),
            "db_dir": str(decryptor.db_dir) if decryptor.db_dir else None
        }, ensure_ascii=False)

    @mcp.tool()
    @handle_tool_exceptions
    def get_sessions(ctx: Context, limit: int = 50) -> str:
        """
        获取所有会话列表。
        返回最近活跃的联系人/群组会话。
        """
        ...
        # 通过 decryptor 获取会话列表
        try:
            sessions = decryptor.get_all_sessions(limit=limit)
    ...
    def query_messages_detail(
        ...
        try:
            messages, total = decryptor.query_messages(contact.username, limit=limit)
            return json.dumps({
                "messages": messages,
                "total": total,
